...
int ParseSymbolTable(const std::string & pPathFileName, SymbolTable_Index_e eSymbolTableWrite, int nSymbolOffset )
{
  ...
  mem_FILE *hFile = mem_fopen_resource (pPathFileName.c_str(), "rt");
	//FILE *hFile = fopen( pPathFileName.c_str(), "rt" );

	if ( !hFile && g_bSymbolsDisplayMissingFile )
	{
		ConsolePrintFormat("Symbol file not found: %s", pPathFileName.c_str());
	...
	if ( hFile )
	{
		while ( !mem_feof(hFile) )
		{
			...
			if ( !mem_fgets(szLine, MAX_LINE-1, hFile) )	// Get next line
			{
				//ConsolePrint("<<EOF");
				break;
			}
...
Update_t CmdSymbolsLoad (int nArgs)
{
	...
	std::string sFileName = (iSymbolTable >= 3 && iSymbolTable <= 6) ? g_sUserDocDir : "";//g_sProgramDir;
...
Update_t CmdSymbolsSave (int nArgs)
{
	...
	strcpy (sFileName, g_sUserDocDir.c_str());
	...
	if (! nArgs)
	{
		pFileName = (char *)g_sFileNameSymbols[ iSymbolTable ];
		strcat (sFileName, pFileName);
	}
