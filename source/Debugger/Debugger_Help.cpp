...
//				case PARAM_CAT_EXPRESSION : // fall-through
				...
				case PARAM_CAT_RANGE      :
					...
					bCategory = false;
					break;
				case PARAM_CAT_DISKSET    : iCmdBegin = CMD_DISKSET_USE     ; iCmdEnd = CMD_DISKSET_D2           ; break;
				default:
					bCategory = false;
					break;
			}
...
//		if (nFound && (! bAllCommands) && (! bCategory))
		if (nFound && (! bAllCommands) && bDisplayCategory)
		{
			...
			if (iCmd <= CMD_DISKSET_D2)
				pCategory = g_aParameters[ PARAM_CAT_DISKSET ].m_sName;
			else
				pCategory = "Unknown!";
...
// View
	...
	// Zero Page pointers
		...
		case CMD_ZEROPAGE_POINTER_7:
			...
			break;
	// Disk set manager
		case CMD_DISKSET_OPEN:
			ConsoleColorizePrint( " Usage: [ <path-name> ]" );
			ConsoleBufferPush( "  Load the disk set by the path name." );
			ConsoleBufferPush( "  Show the name of the active disk set if a path name is not specified." );
			break;
		case CMD_DISKSET_USE:
			ConsoleColorizePrint( " Usage: [ <path-name> ]" );
			ConsoleBufferPush( "  Load the disk set by the path name." );
			ConsoleBufferPush( "  Show the name of the active disk set if a path name is not specified." );
			break;
		case CMD_DISKSET_LIST:
			ConsoleColorizePrint( " Usage: [ * ]" );
			ConsoleBufferPush( "  Display names of disks in the disk set." );
			break;
		case CMD_DISKSET_D1:
			ConsoleColorizePrint( " Usage: [ <disk-name|#num>|EJECT ]" );
			ConsoleBufferPush( "  Insert the disk with the specified name or number into the drive." );
			ConsoleBufferPush( "  Also can eject the currect disk from the drive." );
			ConsoleBufferPush( "  If no cmds specified, will display the name of the current disk." );
			break;
		case CMD_DISKSET_D2:
			ConsoleColorizePrint( " Usage: [ <disk-name|#num>|EJECT ]" );
			ConsoleBufferPush( "  Insert the disk with the specified name or number into the drive." );
