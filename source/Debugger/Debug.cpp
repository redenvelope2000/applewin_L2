...
// Profile
	const int NUM_PROFILE_LINES = (int)NUM_OPCODES + (int)NUM_OPMODES + 16;
...
//===========================================================================
static Update_t GenericLoadDebugData (char *filename)
{
  const std::string sFileName = g_sUserDocDir + filename; // TODO: g_sDebugDir
	Update_t bUpdateDisplay = UPDATE_NOTHING;
  
	g_ConfigState.Reset();
	if ( g_ConfigState.Read(sFileName) ) {
	  char *pText;
	  ...
	  for( iLine = 0; iLine < nLine; iLine++ )
	  {
		  ...
		  if ( pText )
		  {
			  ...
			  int nLineArgs = ParseInput( pText );
			  bUpdateDisplay |= ExecuteCommand( nLineArgs );
		  }
	  }
	  ConsoleBufferPush( TEXT( "Loaded." ) );
	}
	return bUpdateDisplay|ConsoleUpdate();
}
...
Update_t CmdBookmarkLoad (int nArgs)
{
	...
	{
		return GenericLoadDebugData (g_aArgs[ 1 ].sArg);
	}
...
//===========================================================================
static int addr_stack[256], addr_stack_index = -1;
static void reset_address_stack ()
{
  addr_stack_index = -1;
}
// return 0 if succ.
static int push_addr (WORD address)
{
  if (addr_stack_index+1 < sizeof(addr_stack)/sizeof(addr_stack[0])) {
	addr_stack_index++;
	addr_stack[addr_stack_index] = (int)address;
	return 0;
  }
  return -1;
}
// return 0 if succ.
static int pop_addr (WORD &address)
{
  if (addr_stack_index >= 0) {
	address = (WORD) addr_stack[addr_stack_index];
	addr_stack_index--;
	return 0;
  }
  return -1;
...
//===========================================================================
static const char *BreakpointCmd (Breakpoint_t *bp)
{
  if (bp->eSource == BP_SRC_MEM_RW) return "BPM";
  return "BPR";
}
static char *BreakpointRange (Breakpoint_t *bp)
{
  static char range[32];
  char *p = range;
  *p = 0;
  if (bp->nLength == 1) sprintf(p, "%04X", bp->nAddress);
  if (bp->nLength > 1) {
	sprintf(p, "%04X,%04X", bp->nAddress, bp->nLength);
  }
  return range;
}
...
Update_t CmdBreakpointSave (int nArgs)
{
	...
	{
		Breakpoint_t *bp = &g_aBreakpoints[ iBreakpoint ];
		if (bp->bSet)
		{
			strcpy ( sText, BreakpointCmd(bp) ); strcat ( sText, " " );
			if (bp->eSource == BP_SRC_MEM_RW) {
				strcat ( sText, BreakpointRange(bp) );
			} else {
				strcat ( sText, g_aBreakpointSource[bp->eSource] );
				strcat ( sText, g_aBreakpointSymbols[bp->eOperator] );
				strcat ( sText, BreakpointRange(bp) );
			}
			g_ConfigState.PushLine( sText );
			if (! bp->bEnabled)
...
Update_t CmdConfigLoad (int nArgs)
{
	...
	if (nArgs && g_aArgs[ 1 ].bType & TYPE_QUOTED_2) {
...
//===========================================================================
Update_t CmdMinidumpSave (int nArgs)
{
	char sText[ CONSOLE_WIDTH ];
	...
	int iMD = 0;
	while (iMD < NUM_MEM_DUMPS)
	{
		if (g_aMemDump[ iMD ].bActive)
		{
			sprintf( sText, "M%c%d %04X\n"
				, g_aMemDump[iMD].eView == MEM_VIEW_HEX ?'D' :(g_aMemDump[iMD].eView == MEM_VIEW_ASCII ?'A' :'T')
				, iMD+1
				, g_aMemDump[ iMD ].nAddress
			);
			g_ConfigState.PushLine( sText );
		}
		iMD++;
	...
	{
		if (! (g_aArgs[ 1 ].bType & TYPE_QUOTED_2))
			return Help_Arg_1( CMD_WINDOW_DATA_1 );
...
bool ConfigSave_BufferToDisk ( const char *pFileName, ConfigSave_t eConfigSave )
{
	...
	const std::string sFileName = g_sUserDocDir + pFileName; // TODO: g_sDebugDir

  ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - file=%s\n", __FUNCTION__, sFileName.c_str());
	FILE *hFile = fopen( sFileName.c_str(), pMode );
...
Update_t CmdConfigSave (int nArgs)
{
	...
	if (ConfigSave_BufferToDisk( sFilename, CONFIG_SAVE_FILE_CREATE ))
	{
		ConsoleBufferPush( TEXT( "Saved." ) );
		return ConsoleUpdate();
	}
...
//			g_nDisasmBotAddress--;
		...
		iAddress = g_nDisasmTopAddress - 64;
		nLines = 64;
		while (nLines-- > 0)
		{
			_6502_GetOpmodeOpbyte( iAddress, iOpmode, nOpbytes );
			iAddress += nOpbytes;
			if (iAddress == g_nDisasmTopAddress) {
			  bFound = true;
			  g_nDisasmTopAddress -= nOpbytes;
			  break;
			}
		...
		{
			// Singularity
			WORD curLineAddress = g_nDisasmCurAddress;
			g_bDisasmCurBad = true;
			g_nDisasmTopAddress--;
			DisasmCalcCurFromTopAddress();
			DisasmCalcBotFromTopAddress();
			// See if the prev cur address(curLineAddress) is still available.
			nLines = 64;
			WORD prevLineAddress;
			iAddress = g_nDisasmTopAddress;
			while (nLines-- > 0)
			{
				prevLineAddress = iAddress;
				_6502_GetOpmodeOpbyte( iAddress, iOpmode, nOpbytes );
				iAddress += nOpbytes;
				if (iAddress == curLineAddress) {
				  g_bDisasmCurBad = false;
				  break;
				}
			}
...
//===========================================================================
Update_t CmdCursorSetResetBPX (int nArgs)
{
	...
	if (g_nBreakpoints > 0) {
	  ...
	  {
		if (g_aBreakpoints[i].bSet && g_aBreakpoints[i].nAddress == nAddress) {
		  bpx_slot = i;
		  break;
		}
	  }
	...
	if (bpx_slot < 0) {
	  CmdBreakpointAddPC (0);
	} else {
	  _BWZ_RemoveOne( g_aBreakpoints, bpx_slot, g_nBreakpoints );
	}
	return UPDATE_ALL;
}
...
Update_t CmdDisk ( int nArgs)
{
	...
	else
	{
		...
		diskCard.InsertDisk( iDrive, pDiskName, false, IMAGE_DONT_CREATE );
		GetFrame().FrameRefreshStatus(DRAW_LEDS | DRAW_BUTTON_DRIVES | DRAW_DISK_STATUS);
	}
		...
		while ((iPathSeparator = sNewPath.find( PATH_SEPARATOR, iPrevSeparator )) != std::string::npos)
		{
			...
			if ((nSubDirLen == 3) && (sSubDir == UP_DIR))   // Up directory "..\" in the subpath?
			{
				...
				if (nLastSeparator != std::string::npos)
				{
					...
					ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - g_sCurrentDir=%s\n", __FUNCTION__, g_sCurrentDir.c_str());
				}
			}
			else
...
Update_t CmdWatchSave (int nArgs)
{
	...
	ConfigSave_PrepareHeader( PARAM_CAT_WATCHES, CMD_WATCH_CLEAR );
	...
	while (iWatch < MAX_WATCHES)
	{
		...
		{
			sprintf( sText, "%s %04X\n"
				, g_aCommands[ CMD_WATCH_ADD ].m_sName
				, g_aWatches[ iWatch ].nAddress
			);
			g_ConfigState.PushLine( sText );
		}
		iWatch++;
...
Update_t CmdZeroPageSave    (int nArgs)
{
	...
	ConfigSave_PrepareHeader( PARAM_CAT_ZEROPAGE, CMD_ZEROPAGE_POINTER_CLEAR );

	int iZP = 0;
	while (iZP < MAX_ZEROPAGE_POINTERS)
	{
		ZeroPagePointers_t *bp = &g_aZeroPagePointers[ iZP ];
		if (bp->bSet)
		{
			sprintf ( sText, "%s%d %02X"
				  , g_aCommands[ CMD_ZEROPAGE_POINTER ].m_sName
				  , iZP
				  , g_aZeroPagePointers[ iZP ].nAddress
			);
			g_ConfigState.PushLine( sText );
			if (! bp->bEnabled)
			{
				sprintf( sText, "%s %x\n"
					, g_aCommands[ CMD_ZEROPAGE_POINTER_DISABLE ].m_sName
					, iZP
				);
				g_ConfigState.PushLine( sText );
			}
...
//===========================================================================
static bool IsFileNameInAbsolutePath (char *name)
{
  return (name[0] == '/' || 
          name[0] =='\\' || 
...
static int  Insert_Diskette (int drvnum, char *fn, bool silence_flag)
{
	...
	if ((!IsFileNameInAbsolutePath (fn)) // The file name does not have an absolute path.
		&& folder != NULL) {
		name = new char[strlen (folder) + strlen (fn) + 1];
	  strcpy (name, folder);
	  strcat (name, fn);
	} else {
		name = new char[strlen (fn) + 1];
		strcpy (name, fn);
	}

	Disk2InterfaceCard& diskCard = dynamic_cast<Disk2InterfaceCard&>(GetCardMgr().GetRef(SLOT6));
	bool hadDisk = strlen(diskCard.GetFullName(drvnum).c_str()) > 0;
	//res = DiskInsert( drvnum, name, IMAGE_USE_FILES_WRITE_PROTECT_STATUS, IMAGE_DONT_CREATE );
	Disk2InterfaceCard& disk2Card = dynamic_cast<Disk2InterfaceCard&>(GetCardMgr().GetRef(SLOT6));
	res = disk2Card.InsertDisk( drvnum, name, IMAGE_USE_FILES_WRITE_PROTECT_STATUS, IMAGE_DONT_CREATE );
	if (res == eIMAGE_ERROR_NONE) {
		sprintf (sText, "D%d:%s", drvnum+1, fn);
		GetFrame().FrameRefreshStatus(DRAW_LEDS | DRAW_BUTTON_DRIVES);
		ConsoleBufferPush (sText);
		if (!silence_flag) write_wave_pcm_reg(WAVE_PCM_SUPER_CH, hadDisk? 0x5 : 0x3);
	} else {
		sprintf (sText, "D%d:Failed", drvnum+1, fn);
		ConsoleBufferPush (sText);
	}
	delete[] name;
	return res;
...
int Load_Diskset (char *set_name, bool silence_flag)
{
	char *name;
	int res = 0;
	if (!IsFileNameInAbsolutePath (set_name)) {
	  name = new char[strlen (g_sUserDocDir.c_str()) + strlen (set_name) + 1];
	  strcpy (name, g_sUserDocDir.c_str()); // Add a global path.
	  strcat (name, set_name);
	} else {
	  name = new char[strlen (set_name) + 1];
	  strcpy (name, set_name);
	}
	if (DisksetOpen (name) == 0) {
		ConsoleBufferPush( TEXT( "Disk set loaded." ) );
		int d1 = -1, d2 = -1;
		DisksetGetDefault (&d1, &d2);
		if (d1 >= 0) {
		  char *un = NULL, *fn = NULL;
		  DisksetGetItem (d1, &un, &fn);
		  if (fn) {
			Insert_Diskette (0, fn, silence_flag);
		  }
		}
		if (d2 >= 0) {
		  char *un = NULL, *fn = NULL;
		  DisksetGetItem (d2, &un, &fn);
		  if (fn) {
			Insert_Diskette (1, fn, silence_flag);
		  }
		}
		res = 0;
	} else {
	  res = -1;
		ConsoleBufferPush( TEXT( "Failed." ) );
	}
	delete[] name;
	if (silence_flag) ConsoleUpdate();
	return res;
}
Update_t CmdDisksetUse         (int nArgs)
{
	if (nArgs == 1 && g_aArgs[ 1 ].sArg) {// g_aArgs[ 1 ].bType & TYPE_QUOTED_2) {
    Load_Diskset (g_aArgs[ 1 ].sArg, false);
	} else {
		char *diskset_name = DisksetName ();
		if (diskset_name)
			ConsoleBufferPush( DisksetName () );
	}
	return ConsoleUpdate();
}
Update_t CmdDisksetOpen        (int nArgs)
{
	return CmdDisksetUse (nArgs);
}
Update_t CmdDisksetList        (int nArgs)
{
...
static Update_t _cmdDisksetDN          (int nArgs, int drvnum)
{
	int index = -1;
	char *un = NULL, *fn = NULL;
	Disk2InterfaceCard& diskCard = dynamic_cast<Disk2InterfaceCard&>(GetCardMgr().GetRef(SLOT6));
	if (nArgs < 1) {
		//ConsoleBufferPush ( diskCard.GetFullDiskFilename(drvnum).c_str() );
		//ConsoleBufferPush ( diskCard.DiskGetFullPathName(drvnum).c_str() );
		//ConsoleBufferPush ( diskCard.GetFullName(drvnum).c_str() );
		//ConsoleBufferPush ( diskCard.GetBaseName(drvnum).c_str() );
		
		LPCTSTR disk_name = diskCard.GetFullName(drvnum).c_str();
		ConsoleBufferPush ( disk_name && strlen (disk_name) > 0 ?disk_name : "Empty" );
	} else if (nArgs == 1 && g_aArgs[ 1 ].sArg) {
		int iParam = 0;
		FindParam( g_aArgs[ 1 ].sArg, MATCH_EXACT, iParam, _PARAM_DISK_BEGIN, _PARAM_DISK_END );
		if (!(g_aArgs[ 1 ].bType & TYPE_QUOTED_2) && iParam == PARAM_DISK_EJECT) {
      Disk2InterfaceCard& diskCard = dynamic_cast<Disk2InterfaceCard&>(GetCardMgr().GetRef(SLOT6));
		  bool hadDisk = strlen(diskCard.GetFullName(drvnum).c_str()) > 0;
		  if (hadDisk) {
  			diskCard.EjectDisk( drvnum );
  			GetFrame().FrameRefreshStatus(DRAW_LEDS | DRAW_BUTTON_DRIVES);
   			ConsoleBufferPush ( "Ejected." );
  		  write_wave_pcm_reg(WAVE_PCM_SUPER_CH, 0x4);
  		} else {
  		  ConsoleBufferPush ( "No disk." );
  		}
		} else {
			un = g_aArgs[ 1 ].sArg;
			DisksetGetItem (un, &fn);
		}
	} else if (g_aArgs[ 1 ].bType & TYPE_VALUE) {
		index = g_aArgs[ 1 ].nValue;
		DisksetGetItem (index-1, &un, &fn);
		if (fn) {
		  ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - (%s)disk path=%s\n", __FUNCTION__, un, fn);
		}
	}
	
	if (fn) {
		Insert_Diskette (drvnum, fn, false);
	}
	return ConsoleUpdate();
}
Update_t CmdDisksetD1          (int nArgs)
{
	return _cmdDisksetDN (nArgs, 0);
}
Update_t CmdDisksetD2          (int nArgs)
{
	return _cmdDisksetDN (nArgs, 1);
}
...
//	HDC hTmpDC  = CreateCompatibleDC( FrameGetDC() );
  ...
  ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - load ROM symbols\n", __FUNCTION__);
...
//	if (g_hFontWebDings)
  ...
  g_sCurrentDir = g_sUserDocDir;
	...
	if (!doneAutoRun)	// Don't re-run on a VM restart
	{
		...
		// Look in g_sUserDocDir, then g_sProgramDir.
		std::string pathname = g_sUserDocDir + debuggerAutoRunName;
...
//					}
			...
			case VK_UP:
				...
				else
				{
					...
					if (KeybGetShiftStatus())
						...
						if (g_nDisasmCurLine > 0 && g_nDisasmCurLine < (g_nDisasmWinHeight - 1) / 2) {
							g_nDisasmCurLine--;
   							bUpdateDisplay |= UPDATE_CONSOLE_DISPLAY;
						} else {
				...
				else
				{
					...
					if (KeybGetShiftStatus())
						...
						bUpdateDisplay |= CmdCursorLineDown(1);
 						...
 						DisasmCalcTopBotAddress();
						if (g_nDisasmCurLine < (g_nDisasmWinHeight - 1) / 2) {
							g_nDisasmCurLine++;
							bUpdateDisplay |= UPDATE_CONSOLE_DISPLAY;
						} else {
			...
			case VK_RIGHT:
				...
				if (KeybGetAltStatus())
					...
					WORD old_cur_addr = g_nDisasmCurAddress;
					bUpdateDisplay |= CmdCursorFollowTarget( CURSOR_ALIGN_CENTER );
					if (bUpdateDisplay && g_nDisasmCurAddress != old_cur_addr)
						push_addr (old_cur_addr); // Save the old address.
				}
				break;

			case VK_LEFT:
				if (KeybGetCtrlStatus())
					bUpdateDisplay |= CmdCursorSetResetBPX( 0 );		
				else if (KeybGetShiftStatus())
