...
void DrawPointBmp(void *buffer, int x, int y, unsigned int color, int rwidth, int rheight)
{
   int idx;
   ...
   if (x < rwidth && y < rheight) {
      idx = x + y*rwidth;
      #ifdef M16B
         ((Uint16 *)buffer)[idx] = color;
      #else
        ((Uint32 *)buffer)[idx] = color;
      #endif
   }
}
