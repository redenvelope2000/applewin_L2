...
void retro_get_system_info(retro_system_info *info)
{
  ...
  info->valid_extensions = "bin|do|dsk|nib|po|gz|woz|zip|2mg|2img|iie|apl|hdv|yaml|m3u|set";
}
...
void retro_get_system_av_info(retro_system_av_info *info)
{
  ...
  info->timing.fps            = ra2::Game::FPS; // 60
  info->timing.sample_rate    = SPKR_SAMPLE_RATE; // 44100
}

const char *system_resource_directory;

extern uint8_t _binary_resource_zip_start[];
extern uint8_t _binary_resource_zip_end[];

void retro_set_environment(retro_environment_t cb)
{
  ra2::environ_cb = cb;

  cb(RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY, &system_resource_directory);
  ...
  ra2::CreateRetroRegistry();
  ra2::SetupRetroVariables();

  init_mem_FILE ();
  register_mem_FILE ("resource.zip", _binary_resource_zip_start, (long) (_binary_resource_zip_end - _binary_resource_zip_start));
}
...
static void update_retro_vars ()
{
  ...
  GetVideo().SetVideoType((VideoType_e)value);
  GetVideo().VideoReinitialize(false);    

  REGLOAD(TEXT(REGVALUE_ENHANCE_DISK_SPEED), &value);
  GetCardMgr().GetDisk2CardMgr().SetEnhanceDisk(value ? true : false);
}
...
void retro_run(void)
{
  bool retro_vars_updated;
  if (ra2::environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE_UPDATE, &retro_vars_updated) && retro_vars_updated) {
    update_retro_vars();
  }
  ...
  //ra2::writeAudio(ms);
  ra2::writeAudio(SPKR_SAMPLE_RATE/ra2::Game::FPS    // 44100/60 = 735 samples/frame, must be integral.
                  , !ourGame->warm_up_reset_done);   // Effectively shuts up the speaker before the warm-up reset.
}
...
bool retro_load_game(const retro_game_info *info)
{
  ...
  try
  {
    ...
    const std::string disksetEnding = ".set";
    ...
    if (info->path && *info->path)
    {
      ...
      else if (endsWith(gamePath, disksetEnding))
      {
        ok = Load_Diskset((char *)gamePath.c_str(), true) == 0;
      }
      else
      {
        ok = game->getDiskControl().insertDisk(gamePath);
        ra2::log_cb(RETRO_LOG_INFO, "getDiskControl %d\n", ok);
      }
    }
    else
    {
...
UINT64 g_SingleStepping_Cycles = 0;
...
void SingleStep(bool bReinit)
{
  ...
  g_SingleStepping_Cycles += CpuExecute(0, true);
