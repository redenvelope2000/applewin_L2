...
namespace ra2
{
  ...
  {
    const bool writeProtected = false;//IMAGE_FORCE_WRITE_PROTECTED;
  ...
  {
    ra2::log_cb(RETRO_LOG_INFO, "insertHardDisk: %s\n", path.c_str());
    CardManager & cardManager = GetCardMgr();

    if (cardManager.QuerySlot(SLOT7) != CT_GenericHDD)
    {
      ra2::log_cb(RETRO_LOG_INFO, "insertHardDisk: insert generic HDD card to slot7\n");
    ...
    if (harddiskCard)
    {
      ...
      ra2::log_cb(RETRO_LOG_INFO, "insertHardDisk: -> %d\n", bRes);
      return bRes == TRUE;
    }
