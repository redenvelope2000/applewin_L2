...
static bgra_t* g_hConsoleFontFramebits = NULL;
static bgra_t* g_pDebuggerMemFramebits = NULL;
int debugger_framebuffer_width, debugger_framebuffer_height, debugger_framebuffer_pitch;
void *debugger_framebuffer;
	uint8_t g_cConsoleBrushFG_r;
	uint8_t g_cConsoleBrushFG_g;
	uint8_t g_cConsoleBrushFG_b;
	uint8_t g_cConsoleBrushBG_r;
	uint8_t g_cConsoleBrushBG_g;
	uint8_t g_cConsoleBrushBG_b;
...
{
  if (g_pDebuggerMemFramebits == NULL) {
    debugger_framebuffer_width = GetVideo().GetFrameBufferBorderlessWidth();
    debugger_framebuffer_height = GetVideo().GetFrameBufferBorderlessHeight();
    debugger_framebuffer_pitch = debugger_framebuffer_width * sizeof(bgra_t);
    g_pDebuggerMemFramebits = (bgra_t *) new char[debugger_framebuffer_width * debugger_framebuffer_height * sizeof(bgra_t)];
    memset (g_pDebuggerMemFramebits, 0, debugger_framebuffer_width * debugger_framebuffer_height * sizeof(bgra_t));
    debugger_framebuffer = g_pDebuggerMemFramebits;
    ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - debugger frame buffer created, line pitch=%d\n", __FUNCTION__, debugger_framebuffer_pitch);
  }
  return nullptr;
}
...
{
  //if (g_pDebuggerMemFramebits)
  //  delete[] g_pDebuggerMemFramebits;
}

void ReleaseConsoleFontDC(void)
{
  //if (g_hConsoleFontFramebits)
  //  delete[] g_hConsoleFontFramebits;
}

HDC GetConsoleFontDC(void)
{
  if (g_hConsoleFontFramebits == NULL) {
    // Load display font. ID_DEBUGGER_7x8_FONT=Debug_Font32.bmp 112x128 32bpp 
    g_hConsoleFontFramebits = (bgra_t *)new char[0xe000];
    memset (g_hConsoleFontFramebits, 0x5a, 0xe000);
    GetFrame().GetBitmap( "Debug_Font32.bmp"
                         , 0xe000
                         , g_hConsoleFontFramebits);
    ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - debugger font loaded\n", __FUNCTION__);
  }  
  return nullptr;
}
...
// @param glyph Specifies a native glyph from the 7x8 chars Apple Font Texture.
//===========================================================================
...
{	
	...
	for (int yy = 0; yy < CONSOLE_FONT_GRID_Y; yy++)
	{
		...
		index_src -= CONSOLE_FONT_NUM_CHARS_PER_ROW * (int)CONSOLE_FONT_GRID_X;
		index_dst -= DEBUG_VIRTUAL_TEXT_WIDTH * (int)CONSOLE_FONT_GRID_X;
	}*/
	int index_src = (ySrc) * CONSOLE_FONT_NUM_CHARS_PER_ROW * CONSOLE_FONT_GRID_X + xSrc;   // font bitmap
	int index_dst = (yDst) * DEBUG_VIRTUAL_TEXT_WIDTH * CONSOLE_FONT_GRID_X + xDst;   // debugger bitmap
...
void FillBackground(long left, long top, long right, long bottom)
{
	...
	long index_dst = (top) * 80 * CONSOLE_FONT_GRID_X;
...
void test_Debuger_display (int n)
{
  ...
  for (int y=0; y<10; y++) {
    char line[80];
    sprintf (line, "line # %d", y);
    DebuggerPrint (0, y* CONSOLE_FONT_HEIGHT, line);
  }
}
...
//	}
	...
	if (g_bConfigDisasmAddressColon)
	{
		if (bAddressIsBookmark)
		{
			...
			FillBackground(linerect.left, linerect.top, linerect.right, linerect.bottom);
...
static void DrawFlags ( int line, BYTE nRegFlags )
{
	...
	while (nFlag--)
	{
		...
		const char szSet[2] = { static_cast<char>( '0' + static_cast<int>(bSet), '\0' )};
...
void DrawWindowBackground_Main( int g_iWindowThis )
{
	...
	//FillRect( GetDebuggerMemDC(), &rect, g_hConsoleBrushBG );
	FillBackground(rect.left, rect.top, rect.right, rect.bottom);
#endif
}
