...
//	g_fClksPerSpkrSample = g_fCurrentCLK6502 / (double)SPKR_SAMPLE_RATE;
	...
	// . 23 clks @ 1.023MHz
	//221015nao+:note that the (UINT) below will reduce the rate so more samples will be produced which must be regulated later, however
	// it also improves the utilization of RemainderBuffer which helps to stablize the samples production and brings better sounds.
...
static void UpdateSpkr()
{
  if(!g_bFullSpeed || SoundCore_GetTimerState())
  {
    ...
    //ra2::log_cb(RETRO_LOG_INFO, "RA2: %s - %d %d/%llf\n", __FUNCTION__, nNumSamples, nCycleDiff, g_fClksPerSpkrSample);    
...
int SpkrUpdate (DWORD totalcycles)
{
  ...
  if (soundtype == SOUND_WAVE)
  {
	  ...
	  return nSamplesUsed*sizeof(short);
  }
  return 0;
}
