...
public:
	...
	uint64_t m_lastStepperCycle;
	uint64_t m_motorOnCycle;
	BYTE m_headWindow;
	DWORD m_spinning;
	DWORD m_writelight;
	FloppyDisk m_disk;
...
private:
	...
	__inline void IncBitStream(FloppyDisk& floppy);
	...
	void ControlStepperLogging(WORD address, uint64_t cumulativeCycles);
	...
	uint64_t m_diskLastCycle;
	uint64_t m_diskLastReadLatchCycle;
	...
	uint64_t m_deferredStepperCumulativeCycles;
