...
void __stdcall Disk2InterfaceCard::ControlMotor(WORD, WORD address, BYTE, BYTE, ULONG uExecutedCycles)
{
	...
	if (stateChanged)
	{
		...
		if (newState & 1) {
			write_wave_pcm_reg(WAVE_PCM_CH1|WAVE_PCM_LOOP_BIT, 0x0);  // motor on.
		} else {
			write_wave_pcm_reg(WAVE_PCM_CH1|WAVE_PCM_STOP_BIT|WAVE_PCM_FADE_OUT_BIT, 60);	// motor off with 60 frames delay.
		}
	}
...
void Disk2InterfaceCard::ControlStepperDeferred(void)
{
  ...
  if (direction) {
    if (pDrive->m_phase%2) {
      write_wave_pcm_reg(WAVE_PCM_CH0, 0x2);  // Goo~
    }
  }
...
void Disk2InterfaceCard::ControlStepperLogging(WORD address, uint64_t cumulativeCycles)
{
	FloppyDrive* pDrive = &m_floppyDrive[m_currDrive];
